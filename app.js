...
function addToCart(id) { const product = productById(id); if (!isProductAvailable(product) || Number(product.price) <= 0) return; const entry = cart.find(item => item.id === id); entry ? entry.quantity += 1 : cart.push({ id, quantity: 1 }); renderCart(); }
...
function consultProduct(id) { const product = productById(id); if (!product) return; const settings = storeSettings(); const whatsapp = settings.whatsapp || '5500000000000'; window.open(`https://wa.me/${whatsapp}?text=${encodeURIComponent(`Olá, CHUTEIRAS MBM! Quero saber o preço da ${product.name}.`)}`, '_blank', 'noopener'); }
...
function checkout() { if (!cart.length) return; const settings = storeSettings(); const whatsapp = settings.whatsapp || '5500000000000'; const lines = cart.map(item => { const product = productById(item.id); return `• ${product.name} (${item.quantity}x) — ${money.format(product.price * item.quantity)}`; }); const total = cart.reduce((sum, item) => sum + productById(item.id).price * item.quantity, 0); window.open(`https://wa.me/${whatsapp}?text=${encodeURIComponent(`Olá, CHUTEIRAS MBM! Quero fazer este pedido:\n\n${lines.join('\n')}\n\nTotal: ${money.format(total)}`)}`, '_blank', 'noopener'); }

function syncStore() { catalogProducts = getStoreProducts(); refreshBrandFilter(); renderProducts(); renderCart(); const settings = storeSettings(); const whatsapp = settings.whatsapp || '5500000000000'; const message = settings.message || 'Olá, CHUTEIRAS MBM! Preciso de ajuda para escolher uma chuteira.'; document.querySelector('#whatsapp-float').href = `https://wa.me/${whatsapp}?text=${encodeURIComponent(message)}`; }
...
window.addEventListener('storage', event => { if ([MBM_CATALOG_KEY, SETTINGS_KEY].includes(event.key)) syncStore(); }); window.addEventListener('focus', syncStore); syncStore();
